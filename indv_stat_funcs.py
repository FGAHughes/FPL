...
def past_x_performances(element_history, x):
    ...
    element_history['xg_value'] = (element_history.goals_scored * 4 + element_history.expected_goals * 6) / 10
    element_history['xa_value'] = (element_history.assists * 3 + element_history.expected_assists * 7) / 10
    element_history['xgc_value'] = (element_history.goals_conceded * 4.5 + element_history.expected_goals_conceded * 5.5) / 10
    ...
    games_played = (element_history_x_gws['minutes'] != 0).sum()
    ...
    if games_played > 0:
        ...
        # Number of actual games
        n_games = len(element_history_x_gws)
        # Assign weights so the most recent always has weight = x
        weights = np.arange(x - n_games + 1, x + 1)
        # Compute weighted average
        weighted_avg = (element_history_x_gws * weights[:, None]).sum(axis=0) / weights.sum()
        # Add back total starts
        weighted_avg['starts'] = element_starts

        avg_stats_across_x = weighted_avg
    # If they didn't start a game, return 0 for all stats
    else:
        avg_stats_across_x = [0, 0, 0, 0, 0, 0, 0, 0]
